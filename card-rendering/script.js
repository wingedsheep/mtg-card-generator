...
// Sets with icons
const sets_with_icons = ["med", "m3c", "mb2", "9ed", "cmd", "m11", "vma", "8ed", "mkc", "ddp", "m12", "ons", "c18", "eld", "zen", "rvr", "evg", "bro", "bbd", "anb", "mm2", "ha4", "uma", "dds", "dom", "m13", "scg", "otj", "dst", "dsc", "und", "c15", "ddm", "gnt", "big", "mkm", "c19", "m19", "mat", "v14", "tpr", "con", "c17", "exp", "m14", "woe", "mid", "w17", "wwk", "ema", "avr", "iko", "mh1", "afr", "thb", "blc", "tsb", "ddd", "v13", "m21", "ha5", "dsk", "ncc", "ktk", "c16", "khm", "nph", "som", "ddt", "jou", "mb1", "mm3", "mom", "pip", "5dn", "fdc", "fut", "moc", "afc", "ddu", "ltc", "stx", "ths", "mma", "spg", "j22", "ddn", "unh", "xln", "wot", "grn", "lgn", "rix", "dmu", "m15", "clu", "cns", "cc1", "h09", "v09", "v12", "cn2", "ddo", "e02", "v17", "sis", "pd2", "bng", "bot", "otc", "gn3", "cma", "vow", "me2", "lcc", "40k", "dis", "m10", "sta", "2xm", "ha3", "chk", "me3", "mic", "soi", "dka", "v15", "gn2", "war", "j21", "voc", "ddj", "jud", "znr", "ust", "acr", "c13", "ltr", "ddi", "g18", "plc", "rex", "rtr", "w16", "ddc", "ala", "hou", "ori", "fdn", "10e", "c21", "j25", "dd2", "roe", "sir", "ha2", "aer", "cm2", "snc", "ss2", "znc", "ddl", "gpt", "jmp", "v16", "m20", "drb", "v11", "md1", "cmm", "lrw", "bok", "mor", "tsp", "mh2", "ima", "nec", "eve", "dgm", "dde", "dmc", "tor", "brr", "2x2", "neo", "ha1", "rna", "zne", "cmr", "ss1", "bfz", "dtk", "sch", "gtc", "c14", "mh3", "ddg", "mrd", "csp", "a25", "frf", "c20", "ddk", "kld", "akr", "blb", "lci", "v10", "isd", "akh", "sld", "sok", "cm1", "khc", "mul", "who", "ddq", "tsr", "woc", "clb", "dmr", "me4", "klr", "pd3", "one", "onc", "ddf", "hop", "otp", "emn", "gs1", "gdy", "ddh", "arb", "arc", "shm", "e01", "ddr", "pc2", "pca", "brc", "hbg", "ogw", "rav", "mbs"];

/* ======================== Utility: parseOracle ======================== */
/* This function replaces mana symbols with <img> tags and wraps any text in parentheses in a span for italic styling */
...
/* ======================== computeCardProps ======================== */
/* Computes many of the properties from your original Vue code.
   (This version is simplified in some parts; you can expand it as needed.) */
function computeCardProps(card, currentFace) {
    ...
    // is_land: if type_line begins with "Land" or "Terrain"
    props.is_land =
    ...
    // has_legendary_crown: legendary and not planeswalker and not compasslanddfc
    ...
    // mana_cost (using a regex to extract symbols)
    const manaRegex = /{([^}]+)}/g;
    props.mana_cost = [];
    if (props.card_face.mana_cost) {
        let match;
        while ((match = manaRegex.exec(props.card_face.mana_cost)) !== null) {
            if (["archive", "japanese-archive"].includes(props.card_face.art_variant))
                props.mana_cost.push("assets/img/symbols/" + match[1] + ".svg");
            else
                props.mana_cost.push(
                    mana_symbols["{" + match[1] + "}"]
                        ? mana_symbols["{" + match[1] + "}"].svg_uri
                        : ""
                );
        }
    }

    // adventure and modal back mana cost
    ...
    if (props.card_face.mdfc_hint)
        ...
        props.mdfc_hint_text = props.back_face.type_line
            .substr(props.back_face.type_line.indexOf("—") + 1)
            .trim();
    else props.mdfc_hint_text = "";

    // extended art?
    props.extended_art = ["extended", "full", "full-footer"].includes(props.card_face.art_variant);

    // oracle_lines (split by newline and parsed)
    ...
    // levels (for levelup cards) – simplified parsing
    if (props.is_levelup && props.card_face.oracle_text) {
        let levelstext = props.card_face.oracle_text.split(/(LEVEL|NIVEAU) ([^\n]+)\n/);
        let box_color = { W: "#dcc8af", U: "#b9c0da", B: "#af9aa5", R: "#d7a67c", G: "#b2c0b0" }[props.colors ? props.colors[0] : "W"] || "#000000";
        props.levels = [
            {
                level_text: "",
                level: 0,
                power: props.card_face.power,
                toughness: props.card_face.toughness,
                oracle_lines: props.card_face.oracle_text.split("\n").map(parseOracle),
                color: "#00000000",
                box_color: box_color
            }
        ];
        for (let i = 1; i < levelstext.length; i += 3) {
            let pt = props.card_face.power ? levelstext[i + 2].split("\n")[0].split("/") : null;
            let oracle = pt ? levelstext[i + 2].substr(levelstext[i + 2].indexOf("\n") + 1) : levelstext[i + 2];
            props.levels.push({
    ...
    if (props.is_vehicle) props.boxes_colors = "Vehicle";
        ...
        props.boxes_colors =
            props.colors.length > 2 && props.colors.length < 5 ? "Gold" : "Land";
    else props.boxes_colors = props.colors.length > 1 && props.colors.length < 5 ? "Gold" : props.colors;
    ...
    let frameColor =
        props.colors === "Artifact" && props.card_face.colors && props.card_face.colors.length === 1
    ...
    else props.top_line_color = "black";

    // mid_line_color
    if (
        (props.extended_art && !props.is_class && !props.is_planeswalker && (!props.is_transform || currentFace === 0)) ||
        ((props.is_transform || props.is_mdfc) && currentFace === 1 && !props.is_planeswalker)
    )
        props.mid_line_color = "white";
    else props.mid_line_color = "black";
    ...
    // illustration – choose from either card or card_face depending on adventure layout.
    ...
    // set_icon_uri
    console.log("Set Icon: ", card.set_icon);


    if (card.set_icon) props.set_icon_uri = card.set_icon;
    else if (sets_with_icons.includes(card.set))
        props.set_icon_uri = `assets/img/set_icons/${card.set}_${card.rarity || "common"}.webp`;
    else props.set_icon_uri = "";

    console.log("Set Icon URI: ", props.set_icon_uri);

    // archive_frame_colors – simplified
    let archiveColor =
        props.card_face.color_identity && props.card_face.color_identity[0]
            ? props.card_face.color_identity[0]
            : "Gold";
    ...
    // japanese_color and japanese_name_color
    const ArchiveColors = {
        W: "#878167",
        U: "#1c477b",
        B: "#161417",
    ...
    if (props.card_face.colors && props.card_face.colors.length === 1)
        props.japanese_color = ArchiveColors[props.card_face.colors[0]];
    else props.japanese_color = "#927c43";
    if (props.card_face.colors && props.card_face.colors.length === 2)
        props.japanese_name_color = `linear-gradient(to right, ${ArchiveColors[props.card_face.colors[0]]}, ${ArchiveColors[props.card_face.colors[1]]})`;
    else props.japanese_name_color = props.japanese_color;

    // japanese_type_line_border and japanese_oracle_border – simplified SVG Data URLs
    props.japanese_type_line_border = `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='-0.05 -0.05 1.1 1.1'><defs><linearGradient id='gradient' x1='0%' y1='0%' x2='100%' y2='0%'><stop offset='0%' stop-color='${props.japanese_color}' /><stop offset='100%' stop-color='${props.japanese_color}' /></linearGradient></defs><path d='M 0.2 0 c -0.1 0.125 -0.2 0.25 -0.2 0.5 c 0 0.25 0.1 0.375 0.2 0.5 l 0.6 0 c 0.1 -0.125 0.2 -0.25 0.2 -0.5 c 0 -0.25 -0.1 -0.375 -0.2 -0.5 Z' stroke='url(%23gradient)' stroke-width='0.1' fill='none'/></svg>`;
    props.japanese_oracle_border = `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><defs><linearGradient id='gradient' x1='0%' y1='0%' x2='100%' y2='0%'><stop offset='0%' stop-color='${props.japanese_color}' /><stop offset='100%' stop-color='${props.japanese_color}' /></linearGradient></defs><rect x='0' y='0' width='100' height='100' fill='url(%23gradient)' /></svg>`;
    ...
    if (props.colors === "" || props.colors === "Artifact")
        props.frame_colors = { left: FrameColors["Colorless"], right: FrameColors["Colorless"] };
    else if (props.colors.length === 1) {
        props.frame_colors = { left: FrameColors[props.colors[0]], right: FrameColors[props.colors[0]] };
    } else if (props.colors.length === 2) {
        props.frame_colors = { left: FrameColors[props.colors[0]], right: FrameColors[props.colors[1]] };
    } else {
        props.frame_colors = { left: FrameColors["Gold"], right: FrameColors["Gold"] };
    }
...
/* ======================== createMTGCard ======================== */
/* Builds and returns a DOM element representing the card.
   It sets inline CSS custom properties and fills in the card’s inner HTML based on the computed props. */
function createMTGCard(card, scale = 1, renderMargin = 1) {
    const currentFace = 0; // initial face (flip functionality can be added later)
    ...
    cardEl.style.setProperty("--japanese-color", props.japanese_color);
    cardEl.style.setProperty("--japanese-name-color", props.japanese_name_color);
    cardEl.style.setProperty("--japanese-oracle-border", props.japanese_oracle_border);
    cardEl.style.setProperty("--japanese-type-line-border", props.japanese_type_line_border);
    cardEl.style.setProperty("--renderMargin", renderMargin);
    cardEl.style.setProperty("--scale", scale);
    cardEl.style.setProperty("--frame-colors-left", props.frame_colors.left);
    cardEl.style.setProperty("--frame-colors-right", props.frame_colors.right);

    // Set illustration-related CSS variables
    ...
    <div class="top-line">
      ...
      ${props.card_face.printed_name && props.card_face.printed_name !== props.card_face.name
        ? `<div class="original-name-container">...</div>`
        : ""}
    </div>
    <div class="mid-line">
      ...
      ${props.set_icon_uri
        ? `<div class="set-icon-container"><img class="set-icon" src="${props.set_icon_uri}"></div>`
        : ""}
    </div>
    <div class="illustration"></div>
    <div class="oracle normal-oracle">
      ${props.oracle_lines.map(line => `<div class="oracle-inner">${line}</div>`).join("")}
      ${props.card_face.flavor_text
        ? `<div class="oracle-flavor"><hr>${props.card_face.flavor_text}</div>`
        : ""}
    </div>
    ...
    // Attach a dummy _cardInstance for flip functionality
    cardEl._cardInstance = {
        flip: function () {
            alert("Flip functionality not implemented in this demo.");
        }
    };

    // Explicitly set the background of the .illustration div
    const illustrationDiv = cardEl.querySelector(".illustration");
    if (illustrationDiv) {
    ...
    // Add oracle text resizing
    const oracle = cardEl.querySelector('.oracle');
    if (oracle) {
        let fontSize = 1; // Initial font size
        oracle.style.fontSize = `${fontSize}em`;

        const resizeText = () => {
            // Reset font size to the initial value before measuring
            oracle.style.fontSize = `${fontSize}em`;

            // Reduce font size until it fits or reaches a minimum size
            while (oracle.scrollHeight > oracle.clientHeight && fontSize > 0.5) {
                fontSize -= 0.05; // Decrement in small steps
                oracle.style.fontSize = `${fontSize}em`;
            }
        };
        ...
        // Observe changes to the oracle text content
